...
public class OpenApiResolvedType {
	 ...
	 */
	private final Map<String, JsonNode> schemaTypeSection = new LinkedHashMap<>();

	public OpenApiResolvedType(final OpenApiDataType type, final JsonNode node) {
		this.type = type;
		this.node = node;
		final Iterator<Entry<String, JsonNode>> iterator = node.fields();
		iterator.forEachRemaining(entry -> {
			schemaTypeSection.put(entry.getKey(), entry.getValue());
		});
	}
	...
	public Map<String, JsonNode> getSchemaTypeSection() {
		return schemaTypeSection;
	}
}
