 ...
 * capabilities, reimplementing the algorithm, even in a limited way, is far from trivial — especially regarding the handling of the
 * aliases.
 *
 * However, for this plugin to work with Spring, it already requires the spring-web library, which itself has transitive compile
 * dependencies on spring-core, where the annotation resolution mechanism is implemented. (Also on spring-beans, useful for the BeanUtil
 * invocations)
 * The simplest and most efficient approach is therefore to call Spring’s functions via reflection.
 */
public class SpringMergeAnnotationsHelper implements MergeAnnotationsHelper {
	...
	public SpringMergeAnnotationsHelper(ClassLoaderHelper classLoaderHelper) {
